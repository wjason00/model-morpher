...
def mesh_from_sdf_batched(vol_batched, min_bounds, spacing, device, max_workers = 4):
    """
    Extract a mesh from an SDF volume using marching cubes and apply high-quality post-processing.
    ...
    :param vol_batched: Batched SDF volume tensor (batch_size, resolution, resolution, resolution) - should be normalized
    :param min_bounds: World-space coordinate form of grid.
    :param spacing: Spacing between each voxel
    :param device: Torch device
    :return: High-quality PyVista PolyData mesh, or None if extraction fails
    ...
    if isinstance(vol_batched, list):
        vol_batched = torch.stack(vol_batched)

    vol_batched = vol_batched.to(torch.float32)

    try:
        verts_list, faces_list = marching_cubes(vol_batched, isolevel = 0.0)
    except Exception as e:
        # Fallback to CPU
        verts_list, faces_list = marching_cubes(vol_batched.cpu(), isolevel = 0.0)
    ...
    for i in range(len(verts_list)):
        # Convert to NumPy 
        verts = _to_numpy(verts_list[i])
        faces = _to_numpy(faces_list[i])
    ...
    with ThreadPoolExecutor(max_workers = max_workers) as executor:
        futures = {executor.submit(postprocess_mesh, mesh): i
                   for i, mesh in enumerate(meshes) if mesh is not None}
        
        processed_meshes = [None] * len(meshes)

        for future in as_completed(futures):
            index = futures[future]
            processed_meshes[index] = future.result()

    return [mesh for mesh in processed_meshes if mesh]
